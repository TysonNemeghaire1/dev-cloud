...
1. Kubernetes Secrets natifs (stockés dans etcd de GKE)
   Cas d'usage : configurations internes au cluster qui n'ont pas besoin de rotation automatique
   ni d'audit centralisé (ex. clés de sessions éphémères, tokens de service internes).
   Avantage : intégration native K8s, pas de dépendance externe. Limite : chiffrés "at rest"
   seulement si Application-layer encryption est activé ; pas d'historique de versions ni d'audit
   granulaire par défaut.

2. Config Connector (KRM) avec GCP Config Map / Parameter Manager
   Cas d'usage : configurations non-secrètes mais sensibles à l'environnement (URLs d'endpoints,
   feature flags, paramètres de connexion) qui doivent être synchronisées entre GCP et Kubernetes
   via des ressources CRD. Avantage : gestion déclarative GitOps des paramètres GCP directement
   dans les manifests K8s, sans coder les valeurs en dur dans les images.
```
...
gcloud projects add-iam-policy-binding ${PROJECT_ID} \
  --member="serviceAccount:${SA_EMAIL}" \
  --role="roles/container.developer"   # déploiement GKE
...
Une clé JSON est un credential statique longue durée : si elle fuite (dans les logs, un dépôt Git,
un artifact de build), n'importe qui peut l'utiliser depuis n'importe où sans expiration.
Workload Identity Federation élimine ce risque en remplaçant la clé par un échange de tokens OIDC :
GitHub Actions génère un token JWT éphémère signé par GitHub, GCP le vérifie et émet un access
token valide 1 heure seulement, lié à un repository spécifique.

Pour FinSecure concrètement, cela élimine :
- Le risque de fuite de clé via git log (aucune clé dans le dépôt ni dans les secrets GitHub)
- La surface d'attaque d'une clé volée réutilisable indéfiniment depuis n'importe quelle IP
- La charge opérationnelle de rotation périodique des clés JSON
```
...
Le scan est placé après le build car il a besoin de l'image Docker construite pour l'analyser,
et avant le déploiement pour jouer le rôle de "quality gate" : si Trivy détecte une CVE
CRITICAL ou HIGH non corrigée, le pipeline s'arrête avec exit-code 1, le job deploy ne se
déclenche pas (needs: security-scan), et aucune image vulnérable n'atteint GKE.

Si on le plaçait après le déploiement en production :
- L'image vulnérable serait déjà en production, exposée aux attaquants le temps du scan
- Le rollback serait nécessaire (opération risquée avec potentielle interruption de service)
- Pour FinSecure sous DSP2, déployer une image non auditée en production constitue une
  non-conformité réglementaire, même si elle est détectée et corrigée rapidement.
```
...
Pull : le consommateur interroge activement Pub/Sub pour récupérer les messages (polling).
Il contrôle le rythme de consommation, ce qui convient aux workloads qui ont besoin de
backpressure (traitement lent, ressources limitées). C'est ce qu'on utilise ici pour la
subscription finsecure-payment-processor.

Push : Pub/Sub pousse automatiquement chaque message vers un endpoint HTTPS dès qu'il arrive.
Pas besoin de polling, mais le endpoint doit être accessible publiquement et répondre rapidement.

Pour FinSecure :
- Pull → idéal pour la Cloud Function déclenchée par trigger Pub/Sub (Gen2 gère ça nativement)
  ou pour des workers GKE qui consomment à leur propre rythme.
- Push → idéal si on voulait déclencher un webhook externe (ex. notifier un système bancaire
  tiers ou une API REST déjà exposée), sans infra de polling à maintenir.
```
...
Quand une Cloud Function Gen2 lève une exception, Cloud Run retourne un code HTTP 5xx à Eventarc,
qui signale à Pub/Sub que le message n'a pas été traité avec succès. Pub/Sub re-délivre alors
automatiquement le message selon sa retry policy.

Pour un message JSON invalide (payload corrompu), le message ne sera jamais parseable quelle que
soit la tentative : la Function échouerait systématiquement, Pub/Sub re-livrerait indéfiniment
jusqu'à épuisement du max-delivery-attempts (5 dans notre config), puis enverrait le message dans
le Dead Letter Topic. Pendant tout ce temps, des ressources sont consommées inutilement pour des
messages qu'on sait ne jamais pouvoir traiter.

La bonne pratique est donc de logger l'erreur et de retourner sans throw, ce qui fait ACK le
message auprès de Pub/Sub : le message corrompu est considéré traité et ne reviendra plus.
```
...
**Tableau des labels appliqués :**

| Ressource | team | environment | feature | cost-center |
|---|---|---|---|---|
| Artifact Registry `tp4-app-registry` | infra | production | platform | engineering |
...
Ajouter deux labels supplémentaires :
- client : boutiqueA | marketplaceB | ecommerceC (identifie le client final)
- billing-model : pay-per-transaction | flat-rate (le modèle de facturation du client)

Exemple appliqué à la Cloud Function de traitement :
  team=backend, environment=production, feature=payments,
  cost-center=engineering, client=boutiqueA, billing-model=pay-per-transaction

Ainsi dans Cloud Billing → Reports, en groupant par label "client", on obtient le coût
exact de l'infrastructure consommée pour chaque client sur la période. Cela permet de :
1. Générer des rapports de chargeback mensuels par client
2. Identifier quel client génère le plus de coûts (ex. MarketplaceB avec 30k transactions/jour)
3. Ajuster la tarification si un client dépasse les seuils prévus dans son contrat
```
...
Quick wins (< 1 jour) :
- Identifier et éteindre les ressources inutilisées via Cloud Recommender (instances idle, disques
  non attachés, IPs réservées non utilisées)
- Réduire les min-instances des Cloud Functions à 0 si ce n'est pas déjà fait
- Vérifier si des environnements de dev/staging tournent inutilement en dehors des heures de bureau
  et les éteindre (ou programmer un arrêt automatique)
- Désactiver les APIs non utilisées pour éviter des scans/appels facturés en arrière-plan

Actions structurelles (à planifier) :
- Activer les Committed Use Discounts (CUD) sur GKE si le workload est stable et prévisible
- Implémenter l'autoscaling vertical sur GKE Autopilot pour rightsizer les pods
- Migrer les jobs batch nocturnes vers Cloud Run Jobs (facturation à la seconde, pas de coût idle)
- Mettre en place des quotas par namespace/équipe pour éviter les dérives futures
- Analyser les transferts de données inter-régions (egress) souvent invisibles mais coûteux
```
...
Non, un CUD 3 ans est probablement prématuré pour FinSecure à ce stade. Facteurs à analyser :

Arguments contre le CUD 3 ans :
- Croissance forte = besoins en ressources imprévisibles : les 8 vCPU d'aujourd'hui peuvent
  devenir 30 vCPU dans 18 mois si les volumes de transactions triplent
- Risque de sur-engagement : si la croissance ralentit ou si l'architecture évolue
  (migration vers plus de serverless), on paye pour des ressources sous-utilisées
- Flexibilité technologique : un engagement 3 ans réduit la capacité à migrer vers de
  nouvelles régions GCP ou de nouveaux types d'instances

Approche recommandée :
- CUD 1 an sur les ressources stables et prévisibles (Cloud SQL en particulier, dont la
  taille change peu quelle que soit la charge applicative) → économie de 25% immédiate
- Pas de CUD sur GKE Autopilot tant que le workload n'est pas stabilisé (attendre 6 mois
  de métriques de consommation)
- Réévaluer un CUD 3 ans après la Série B quand le volume est prévisible sur 3 ans
```
...
env:
  - name: REDIS_HOST
    value: "10.241.113.147"   # IP de l'instance Memorystore finsecure-cache
  - name: REDIS_PORT
    value: "6379"
```
...
|---|---|---|---|
| Latence moyenne | 205 ms | 4.9 ms | 42 x |
| Latence p99 | 217 ms | 10.1 ms | 21 x |
| Requêtes/seconde | 48.7 | 1925.9 | 39 x |
...
Sans invalidation manuelle, BoutiqueD n'apparaîtra dans l'API qu'à l'expiration du TTL, soit
au maximum 1 heure après son inscription (si le cache a été peuplé juste avant l'inscription,
le délai peut atteindre 59 minutes 59 secondes).

Stratégie d'invalidation recommandée — invalidation événementielle (event-driven) :
Lors de toute écriture en base de données (POST /merchants, PUT /merchants/:id, DELETE),
appeler explicitement invalidateCache('merchants:all') avant de retourner la réponse.
C'est ce que fait déjà le endpoint POST /merchants dans notre implémentation.

Pour aller plus loin en production :
- Utiliser un pattern Write-Through : écrire simultanément en DB et en cache à chaque
  modification, pour ne jamais avoir de cache miss après une écriture
- Ou émettre un événement Pub/Sub à chaque modification de marchand, et faire écouter
  un subscriber qui invalide le cache — utile si plusieurs instances de l'API tournent
  et qu'il faut invalider le cache sur tous les pods simultanement
```
