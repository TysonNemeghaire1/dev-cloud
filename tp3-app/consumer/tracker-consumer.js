...
const consumer = kafka.consumer({ groupId: 'tracker-service-group' });
  ...
  await consumer.run({
    ...
    eachMessage: async ({ topic, partition, message }) => {
      ...
      if (topic === 'truck-positions') {
        ...
        if (value.speed_kmh < 5) await publishAlert(value.truck_id, 'TRUCK_STOPPED', value);
        if (value.fuel_level < 20) await publishAlert(value.truck_id, 'LOW_FUEL', value);
