...
Problème 1 — Perte d'état au rebalancing. Les 6 partitions de `transactions-raw`
sont réparties entre les 3 réplicas par le consumer group. La clé = account_id
garantit qu'un compte va toujours sur la même partition, donc un seul réplica
l'analyse à un instant T. Mais dès qu'un rebalance survient (scale, crash,
redéploiement, éviction Autopilot), les partitions sont réassignées à d'autres
réplicas qui n'ont PAS l'historique en mémoire : la fenêtre du compte repart de
zéro et on rate les seuils (une attaque en cours devient invisible).

Problème 2 — État volatile, aucune tolérance aux pannes. `windowedTransactions`
est une Map en mémoire. Si un pod redémarre (OOM, rollout, éviction), tout
l'historique des fenêtres est perdu. Pas de persistance, pas de restauration,
pas de partage entre réplicas → impossible de scaler horizontalement de façon
fiable.

Solution recommandée — State Store externe/persistant :
- Redis partagé entre les réplicas : un sorted set par compte (score = timestamp)
  avec TTL = taille de fenêtre, opérations atomiques (ZADD / ZCOUNT). L'état est
  centralisé, survit aux redémarrages et permet le scaling horizontal.
- OU le mécanisme natif de Kafka Streams : state store RocksDB local adossé à un
  changelog topic Kafka compacté (+ standby replicas). L'état local est restauré
  depuis le changelog au rebalancing → tolérance aux pannes intégrée.
Pour cette stack Node.js, Redis est le plus simple ; pour une vraie robustesse
production, migrer vers la lib Kafka Streams (windowed store + changelog).
```
...
PythonOperator — la tâche s'exécute DANS le worker Airflow lui-même. Conséquences
pour un job de 8 Go + 2 GPU :
- Il faudrait dimensionner TOUS les workers Airflow avec 8 Go RAM + 2 GPU en
  permanence, alors que le réentraînement n'a lieu qu'une fois par jour → gaspillage
  massif de ressources (GPU immobilisé 23h/24) et coût élevé.
- Les dépendances ML lourdes (CUDA, PyTorch, drivers GPU…) devraient être installées
  dans l'image du worker, polluant l'environnement Airflow et créant des conflits de
  versions avec les autres DAGs.
- Un pic mémoire fait tomber le worker (OOM) → ce sont toutes les autres tâches du
  worker qui échouent, pas seulement le réentraînement.

KubernetesPodOperator / GKEStartPodOperator — lance un POD dédié et éphémère :
- Ressources à la demande : le pod réclame ses 8 Gi + 2 GPU sur un node pool GPU
  adapté, puis est détruit en fin de tâche → on ne paie le GPU que pendant l'entraînement.
- Isolation des dépendances : image Docker ML séparée (fraud-ml-trainer), aucun
  impact sur l'environnement Airflow.
- Robustesse : le worker Airflow ne fait que superviser le pod (empreinte légère),
  un crash du job n'affecte pas le scheduler ni les autres tâches.
- Scalabilité indépendante worker ↔ entraînement.
En résumé : isolation, élasticité des ressources (GPU non immobilisé), maîtrise des
coûts et résilience → KubernetesPodOperator est nettement préférable pour une tâche
lourde et ponctuelle.
```
...
|---|---|---|---|
| Transactions/s en régime normal | ~5 tx/s (4,93) | < 50 | ✅ OK |
| Transactions/s pendant l'attaque | ~6 tx/s (pic 6,13) | Détecté si > 200 | ⚠️ Volume bas — l'attaque est détectée par **pattern** (micro-transactions/vélocité), pas par le débit global (le simulateur ne dépasse pas 200 tx/s) |
| Consumer lag fraud-detector | ≈ 0 (−3) | < 100 | ✅ OK (le détecteur suit le flux en temps réel) |
| Latence détection P99 | ~48 ms (histogramme `fraud_detection_latency_seconds`, Panel 4) | < 500ms | ✅ OK |
| Alertes générées en 10 min | ~3700 | — | — |

> Note : le nombre élevé d'alertes (~3700/10 min) vient des seuils de l'énoncé
> (`VELOCITY_THRESHOLD = 20` tx / 5 min) face à un trafic légitime de ~5 tx/s par
> compte → `HIGH_VELOCITY` se déclenche aussi sur les comptes légitimes (beaucoup de
> faux positifs). La vraie cible (micro-transactions ACC-9999) reste isolée par
> `MICRO_TRANSACTION_PATTERN`. Fidèle aux paramètres de l'énoncé.
...
Origine du lag : un seul détecteur (replicas=1) traite séquentiellement tout le flux
→ goulot d'étranglement. Pendant le rattrapage, les nouvelles transactions attendent
en file et l'attaque n'est pas vue à temps.

Architecture cible (latence < 2 s même sous charge) :

1. Paralléliser la consommation. Augmenter le nombre de partitions de
   `transactions-raw` et scaler le détecteur horizontalement (≈ 1 consumer par
   partition). La clé = account_id préserve l'ordre par compte. PRÉREQUIS : régler
   d'abord l'état partagé (cf. Q1.3) en passant sur un state store externe (Redis)
   ou Kafka Streams, sinon le scaling casse la détection.

2. Rendre le traitement O(1) par message. Au lieu de garder des listes en mémoire,
   incrémenter des compteurs de fenêtre dans Redis avec opérations atomiques
   (INCR + EXPIRE, ou ZADD/ZCOUNT sur une fenêtre glissante). Le coût par message
   devient constant → le détecteur suit le débit.

3. Autoscaling piloté par le lag. KEDA (ScaledObject sur le consumer group lag)
   ajoute des réplicas du détecteur dès que le lag dépasse un seuil, AVANT qu'il
   n'explose. Dimensionner la capacité au-dessus du débit de pointe (200 tx/s).

4. Séparer fast path / slow path. Un « fast path » ne fait que la détection peu
   coûteuse (vélocité, micro-transactions, IP) avec une latence garantie ; les
   analyses lourdes (ML, agrégats 30 j) partent vers un « slow path » asynchrone
   (Airflow batch). On ne bloque jamais la détection critique.

5. Idéalement, vrai moteur de streaming (Kafka Streams ou Apache Flink) avec state
   store RocksDB local, windowing natif et exactly-once : scale linéairement avec
   les partitions, garde le lag ≈ 0 et la latence sous la seconde.

Synthèse : state store partagé (Redis) + détecteur scalable horizontalement
(1 pod/partition) + comptage incrémental O(1) + autoscaling KEDA sur le lag,
dimensionné pour le pic → le lag reste bas → latence de détection < 2 s.
```
